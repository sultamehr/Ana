...
#
#   Generated Mon Aug 12 17:20:14 2019  by sultana
#
#====================================
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "CCQENu" = ""; then \
	    if test -d ../CCQENu; then \
	      echo "Installing files from more ../CCQENu to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/CCQENu; then mkdir -p $(install_include_dir)/CCQENu; fi;\
	        $(install_command) ../CCQENu $(install_include_dir);\
	    else \
	      echo "No more CCQENu include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/CCQENu"; \
	  $(uninstall_command) "$(install_include_dir)/CCQENu/*" ; \
	  if test -f $(install_include_dir)/CCQENu.cmtref ; then \
	    echo "Removing $(install_include_dir)/CCQENu.cmtref"; \
	    eval rm -f $(install_include_dir)/CCQENu.cmtref ; \
	  fi; \
	fi
